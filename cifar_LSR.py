...
parser.add_argument('--netName', default='PreActResNet50', choices=model_names, type=str, help='choosing network')
...
if args.resume:
    ...
    checkpoint_path = './checkpoint/ckpt_LSR_cifar_'+str(args.cifar)+'_'+args.netName+'.t7'
...
criterion = LabelSmoothingLoss(label_smoothing=0.1, tgt_vocab_size=args.cifar)
...
def train(epoch):
    ...
    file_path='../records/cifar100/LSR_cifar_' + str(args.cifar) + '_' +args.netName+'_train.txt'
...
def test(epoch):
    ...
    file_path = '../records/cifar100/LSR_cifar_' + str(args.cifar) + '_' +args.netName+ '_test.txt'
    ...
    if acc > best_acc:
        ...
        if not os.path.isdir('checkpoint'):
            os.mkdir('checkpoint')
        save_path = './checkpoint/ckpt_LSR_cifar_' + str(args.cifar) + '_' + args.netName + '.t7'
...
statis_path = '../records/cifar100/STATS_LSR_'+args.netName+'.txt'
