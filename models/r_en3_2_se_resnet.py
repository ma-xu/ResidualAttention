...
class REN32SELayer(nn.Module):
    def __init__(self,in_channel, channel, reduction=16):
        super(REN32SELayer, self).__init__()
    ...
    def forward(self, x):
        ...
        if x[1] is None:
            ...
            all_att = torch.sigmoid(self.fc(y)) + torch.sigmoid(pre_att)
            y = all_att.view(b, c, 1, 1)
...
class PreActBlock(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride=1):
        ...
        self.se = REN32SELayer(in_channel=in_planes, channel=planes*self.expansion)
